...
def non_jit_dust_plume(stardata):
    '''
    Parameters
    ----------
    stardata : dict
...
def np_rotate_x(angle):
    arr = jnp.array([[1, 0, 0],
                     [0, jnp.cos(angle), jnp.sin(angle)],
                     [0, -jnp.sin(angle), jnp.cos(angle)]])
    return arr

def np_rotate_y(angle):
    arr = jnp.array([[jnp.cos(angle), 0, -jnp.sin(angle)],
                     [0, 1, 0],
                     [jnp.sin(angle), 0, jnp.cos(angle)]])
    return arr

def np_rotate_z(angle):
    arr = jnp.array([[jnp.cos(angle), jnp.sin(angle), 0],
                     [-jnp.sin(angle), jnp.cos(angle), 0],
                     [0, 0, 1]])
    return arr
def np_euler_angles(coords, Omega, i, w):
    return np_rotate_z(np.deg2rad(Omega)) @ (
            np_rotate_x(np.deg2rad(i)) @ (
            np_rotate_z(np.deg2rad(w)) @ coords))

def np_kepler_starter(mean_anom, ecc):
    ome = 1 - ecc
    M2 = np.square(mean_anom)
    alpha = 3 * np.pi / (jnp.pi - 6 / jnp.pi)
    alpha += 1.6 / (jnp.pi - 6 / jnp.pi) * (jnp.pi - mean_anom) / (1 + ecc)
    d = 3 * ome + alpha * ecc
    alphad = alpha * d
    r = (3 * alphad * (d - ome) + M2) * mean_anom
    q = 2 * alphad * ome - M2
    q2 = np.square(q)
    w = np.square(jnp.cbrt(np.abs(r) + np.sqrt(q2 * q + r * r)))
    return (2 * r * w / (np.square(w) + w * q + q2) + mean_anom) / d
def np_kepler_refiner(mean_anom, ecc, ecc_anom):
    ome = 1 - ecc
    sE = ecc_anom - np.sin(ecc_anom)
    cE = 1 - np.cos(ecc_anom)
...
def np_kepler_solver_impl(mean_anom, ecc):
    mean_anom = mean_anom % (2 * jnp.pi)

    # We restrict to the range [0, pi)
    high = mean_anom > jnp.pi
    mean_anom = np.where(high, 2 * jnp.pi - mean_anom, mean_anom)

    # Solve
    ecc_anom = np_kepler_starter(mean_anom, ecc)
    ecc_anom = np_kepler_refiner(mean_anom, ecc, ecc_anom)

    # Re-wrap back into the full range
    ecc_anom = np.where(high, 2 * jnp.pi - ecc_anom, ecc_anom)

    return ecc_anom
def np_kepler(mean_anom, ecc):
    E = np_kepler_solver_impl(mean_anom, ecc)
    return E, 2 * np.arctan2(np.sqrt(1 + ecc) * np.sin(E / 2), np.sqrt(1 - ecc) * np.cos(E / 2))


def np_dust_circle(i_nu, stardata, theta, plume_direction, widths):
    '''
    '''
    # print(i_nu)
    i, nu = i_nu
    x = nu / (2 * jnp.pi)
    transf_nu = 2 * jnp.pi * (x + jnp.floor(0.5 - x))
    turn_on = np.deg2rad(stardata['turn_on'])
    turn_off = np.deg2rad(stardata['turn_off'])
    turned_on = np.heaviside(transf_nu - turn_on, 0)
    turned_on *= np.heaviside(widths[i] - stardata['nuc_dist'] * AU2km, 1)   # nucleation distance (no dust if less than nucleation dist), converted from AU to km
    turned_off = np.heaviside(turn_off - transf_nu, 0)
    direction = plume_direction[:, i] / np.linalg.norm(plume_direction[:, i])
    
    half_angle = np.deg2rad(stardata['open_angle']) / 2

    circle = np.array([np.ones(len(theta)) * np.cos(half_angle), 
                        np.sin(half_angle) * np.sin(theta), 
                        (1 - stardata['oblate']) * np.sin(half_angle) * np.cos(theta)])
    circle *= widths[i]
    ### below attempts to model latitude varying windspeed -- don't see this significantly in apep
    ### if you think about it, the CW shock occurs more or less around the equatorial winds so it shouldnt have a huge effect
    # latitude_speed_var = jnp.array([jnp.ones(len(theta)), 
    ...
    valid_dists = np.heaviside(stardata['opt_thin_dist'] - stardata['nuc_dist'], 1)
    t_noaccel = stardata['nuc_dist'] * AU2km / stardata['windspeed1']
    t_linear = 2 * np.sqrt(valid_dists * (stardata['opt_thin_dist'] - stardata['nuc_dist']) * AU2km / (2 * stardata['acc_max']/yr2s))
    accel_lin = np.heaviside(spiral_time - t_noaccel, 0)
    dist_accel_lin = accel_lin * 0.5 * stardata['acc_max']/yr2s * np.min(np.array([spiral_time, t_linear]))**2
    # accel_r2 = jnp.heaviside(spiral_time - t_linear, 0)
    ...
    ### now rotate the circle to account for the star orbit direction
    angle_x = -np.arctan2(direction[1], direction[0]) + jnp.pi
    circle = np_rotate_z(angle_x) @ circle
    
    # circle *= turned_on * turned_off
    weights = np.ones(len(theta)) * turned_on * turned_off
    ...
    alpha = np.deg2rad(stardata['comp_incl'])
    beta = np.deg2rad(stardata['comp_az'])
    comp_halftheta = np.deg2rad(stardata['comp_open'] / 2)
    x = circle[0, :]
    y = circle[1, :]
    z = circle[2, :]
    r = np.sqrt(x**2 + y**2 + z**2)
    particles_alpha = np.arccos(z / r)
    particles_beta = np.sign(y) * np.arccos(x / np.sqrt(x**2 + y**2))
    
    ### to get angular separation of the points on the sphere, I used the cos(alpha) = ... formula from
    # https://www.atnf.csiro.au/people/Tobias.Westmeier/tools_spherical.php#:~:text=The%20angular%20separation%20of%20two%20points%20on%20a%20shpere&text=cos(%CE%B1)%3Dcos(%CF%911)cos(,%CF%861%E2%88%92%CF%862).
    term1 = np.cos(alpha) * np.cos(particles_alpha)
    term2 = np.sin(alpha) * np.sin(particles_alpha) * np.cos(beta - particles_beta)
    angular_dist = np.arccos(term1 + term2)
    ...
    #                                  (1 - stardata['comp_reduction'] * jnp.ones(len(weights))), jnp.ones(len(weights)))
    ## gaussian scaling for companion photodissociation
    comp_gaussian = 1 - stardata['comp_reduction'] * np.exp(-0.5 * (angular_dist / comp_halftheta)**2)
    comp_gaussian = np.maximum(comp_gaussian, np.zeros(len(comp_gaussian))) # need weight value to be between 0 and 1
    companion_dissociate = np.where(angular_dist < photodis_prop * comp_halftheta,
                                      comp_gaussian, np.ones(len(weights)))
    ...
    # now calculate the weights of each point according the their orbital variation
    val_orb_sd = np.max(jnp.array([stardata['orb_sd'], 0.01]))
    prop_orb = 1 - (1 - stardata['orb_amp']) * np.exp(-0.5 * (((transf_nu*180/jnp.pi + 180) - stardata['orb_min']) / val_orb_sd)**2) # weight proportion from orbital variation
    
    # now from azimuthal variation
    val_az_sd = np.max(np.array([stardata['az_sd'], 0.01]))
    prop_az = 1 - (1 - stardata['az_amp']) * np.exp(-0.5 * ((theta * 180/jnp.pi - stardata['az_min']) / val_az_sd)**2)
    
    # we need our orbital proportion to be between 0 and 1
    prop_orb = np.min(np.array([prop_orb, 1]))
    prop_orb = np.max(np.array([prop_orb, 0]))
    # and the same for our azimuthal proportion
    prop_az = np.minimum(np.maximum(prop_az, jnp.zeros(len(prop_az))), jnp.ones(len(prop_az)))
    weights *= prop_orb * prop_az
    
    weights *= companion_dissociate
    
    circle = np.array([circle[0, :], 
                        circle[1, :], 
                        circle[2, :],
                        weights])
...
def np_dust_plume_sub(theta, times, n_orbits, period_s, stardata):
    ...
    E, true_anomaly = np_kepler(2 * jnp.pi * times / period_s, jnp.array([ecc]))
    ...
    positions1 = np.array([np.cos(true_anomaly), 
                            np.sin(true_anomaly), 
                            np.zeros(n_time)])
    ...
    widths = stardata['windspeed1'] * period_s * (n_orbits - np.arange(n_time) / n_t)
    
    plume_direction = positions1 - positions2               # get the line of sight from first star to the second in the orbital frame
    
    # ntimes = np.arange(n_time)
    # inputthings = np.array([[ntimes[i], true_anomaly[i]] for i in range(len(ntimes))])
    # print(inputthings[0])
    # # particles = np.vectorize(lambda i_nu: np_dust_circle(i_nu, stardata, theta, plume_direction, widths))(inputthings)
    # particles = map(lambda i_nu: np_dust_circle(i_nu, stardata, theta, plume_direction, widths), inputthings)
    particles = np.zeros((len(theta), 4, n_time))
    for i in range(n_time):
        particles[:, :, i] = np_dust_circle((i, true_anomaly[i]), stardata, theta, plume_direction, widths).T
    ...
    particles = np.array([np.ravel(particles[:, 0, :]),
                           np.ravel(particles[:, 1, :]),
                           np.ravel(particles[:, 2, :])])
    

    ### the shock originates from the second star, not the WR, so we need to add its position to the spiral
    shock_start = positions2
    shock_start = np.repeat(shock_start, len(theta), axis=-1)
    particles += shock_start

    particles = np_euler_angles(particles, stardata['asc_node'], stardata['inclination'], stardata['arg_peri'])

    return 60 * 60 * 180 / jnp.pi * np.arctan(particles / (stardata['distance'] * 3.086e13)), weights

def np_dust_plume(stardata):
    '''
    Parameters
    ----------
    stardata : dict
    ...
    n_particles = n_points * n_t * n_orbits
    n_time = n_t * n_orbits
    theta = 2 * jnp.pi * np.linspace(0, 1, n_points)
    times = period_s * np.linspace(phase, n_orbits + phase, n_time)
    particles, weights = np_dust_plume_sub(theta, times, n_orbits, period_s, stardata)
    return particles, weights
...
import time
n = 50
numpytest = np.zeros(n)
jitted = np.zeros(n)
non_jitted = np.zeros(n)

for i in range(n):
    t1 = time.time()
    particles, weights = np_dust_plume(wrb.apep)
    numpytest[i] = time.time() - t1
    print(i)

for i in range(n):
    t1 = time.time()
    particles, weights = non_jit_dust_plume(wrb.apep)
    non_jitted[i] = time.time() - t1
    print(i)
...
for i in range(n):
    t1 = time.time()
    particles, weights = dust_plume(wrb.apep)
    jitted[i] = time.time() - t1
    print(i)

fig, ax = plt.subplots()

ax.hist(numpytest, bins=20, label='Numpy')
ax.hist(jitted, bins=20, label='JAX JIT Compilation')
ax.hist(non_jitted, bins=20, label='JAX Vectorisation')

ax.set(xscale='log', xlabel='Model Compute Time (s)')
ax.legend()
fig.savefig('Compute-Time-Comparison.png', dpi=400, bbox_inches='tight')
